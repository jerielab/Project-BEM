...
<head>
    ...
    <style>
        ...
        .velorah-scene-content.is-fading { opacity: 0; transform: scale(0.94); }

        .velorah-flash {
            position: fixed; inset: 0; background: #fff; opacity: 0;
            pointer-events: none; transition: opacity 220ms ease; z-index: 60;
        }
        .velorah-flash.is-visible { opacity: 1; }

        @media (prefers-reduced-motion: reduce) {
            .velorah-video, .velorah-scene-content, .velorah-flash { transition: none !important; }
        }
    </style>
</head>
<body class="velorah-login min-h-screen overflow-x-hidden bg-[#032a42] text-white">
    <main x-data="loginForm()" class="relative isolate flex min-h-screen flex-col overflow-hidden">
        <video
            ...
            class="velorah-video absolute inset-0 -z-10 h-full w-full object-cover"
        ...
        <div class="velorah-flash" x-bind:class="{ 'is-visible': flashing }"></div>
        ...
        <section class="relative z-10 mx-auto flex w-full max-w-7xl flex-1 items-center justify-center px-6 pb-20 pt-10 sm:pb-28" aria-labelledby="login-title">
            <div
                ...
                class="velorah-scene-content grid w-full max-w-5xl -translate-y-3 items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(360px,420px)] lg:gap-16"
            >
                ...
                <div class="animate-fade-rise-delay-2 liquid-glass rounded-[1.5rem] p-5 text-left sm:p-7">
                    ...
                    <form method="POST" action="{{ route('login') }}" class="space-y-5" x-on:submit.prevent="submit($event)">
                        ...
                        <div>
                            ...
                            <input id="email" name="email" type="email" value="{{ old('email') }}" required autofocus autocomplete="username" inputmode="email" class="velorah-input w-full rounded-xl px-4 py-3 text-base text-white outline-none" placeholder="you@example.com" aria-describedby="email-error">
